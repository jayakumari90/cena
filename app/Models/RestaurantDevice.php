...
class RestaurantDevice extends Model
{
   /**
     ...
     * @var string
     */
    protected $table = 'restaurant_devices';
    ...
    public static function manageDeviceIdAndToken($user_id, $device_id, $device_type,$device_uniqueid,$donot_ask='', $methodName) {
        ...
        if ($methodName == 'add') {
            ...
            $row = RestaurantDevice::where(['user_id' => $user_id,'device_uniqueid' => $device_uniqueid])->first();
        ...
        if ($methodName == 'remove') {
            RestaurantDevice::where('user_id', $user_id)
     ...
     public static function getAndUpdateBadgecount($device_id){
        $badge_count = 0;
        $device_row = RestaurantDevice::where('device_id', $device_id)->first();
        if(!empty($device_row)){
